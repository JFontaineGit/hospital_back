...
def set_or_update_google_user(user: User, user_data: dict) -> None:
    ...
    except (NoneResultException, Exception) as e:
        console.print_exception(show_locals=True)
        console.print(f"[yellow]Storage error in get_by_parameter: {e}. Skipping get, but setting new.[/yellow]")
        item = None
    if item:
...
class UserRepository:
    ...
    def create_google_user(user_data: dict) -> Tuple[User, bool]:
        ...
        if missing:
            raise ValueError(f"Missing Google data keys: {missing}")
        ...
        dni = str(user_data.get('id', '00000000')) 
